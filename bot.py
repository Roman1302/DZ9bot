...
@bot.message_handler(func= lambda message: message.text =='Игра' or message.text =='/game')

def message_x0(message):
    bot.send_message(message.from_user.id, f'Игра "Крестики-Нолики"\n{X0.creating_field(list(range(1,10)))}')
    bot.register_next_step_handler(message, X0.cell_selection, message_x01)
    global idRat
    idRat = message.chat.id
    print(message.chat.id)
def message_x01(message):
    global firstnum
    firstnum = message.text
    print("1", firstnum)
    bot.send_message(message.from_user.id, 'Выбирите действие', reply_markup=markup)
...
# def game(message):
#     bot.send_message(message.chat.id, f'Игра "Крестики-Нолики"\n{X0.creating_field(list(range(1,10)))}')
#     # bot.send_message(message.from_user.id, , )
#     bot.register_next_step_handler(message, message_x01(message))
#
# def message_x01(message):
#     bot.send_message(message.from_user.id, "12", X0.cell_selection('x'))
#     bot.register_next_step_handler(message, message_x02)
#     global idRat
#     idRat = message.chat.id
#     print(message.chat.id)
#
# def message_x02(message):
#     global firstnum
#     firstnum = message.text
#     print("1", firstnum)
#
#     bot.send_message(message.from_user.id, 'Выбирите действие')
#     bot.register_next_step_handler(message, message_reply2)
#
# def message_reply2(message):
#     global operat
